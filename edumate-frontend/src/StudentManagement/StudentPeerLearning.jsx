...
export default function StudentPeerLearning({
  ...
  // Suggested Peers are specifically those marked as isSuggested (and who are not incoming/connected)
  const suggestedPeersList = peerLearningPeers.filter((p) => 
    p.isSuggested && p.status !== 'incoming' && p.status !== 'connected'
  )
  
  // Other Peers are those NOT marked as isSuggested (and who are not incoming/connected)
  const otherPeersList = peerLearningPeers.filter((p) => 
    !p.isSuggested && p.status !== 'incoming' && p.status !== 'connected'
  )

  // Only other peers are filtered by the search bar
  const searchedOtherPeers = otherPeersList.filter((peer) => {
  ...
  const renderPeerCard = (peer) => (
    <div key={peer.id} className="p-5 rounded-3xl bg-slate-950/80 border border-slate-800 shadow-sm hover:border-cyan-500/20 transition-all relative overflow-hidden flex flex-col justify-between">
      {peer.isSuggested && (
        <div className="absolute top-0 right-0 bg-cyan-500 text-slate-955 text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-bl-xl z-10">
          Best Match
        </div>
      )}
      <div>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h4 className="font-semibold text-white text-sm flex items-center gap-2">
              {peer.name}
              {peer.hasUnreadMessage && (
                <span className="h-2 w-2 rounded-full bg-cyan-400 animate-pulse" title="New message in inbox" />
              )}
            </h4>
            <p className="text-xs text-slate-400 mt-0.5">Expert in {peer.expertise}</p>
          </div>
          <span className={`px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-[0.2em] ${peer.status === 'connected' ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/20' : peer.status === 'requested' ? 'bg-amber-500/15 text-amber-300 border border-amber-500/20' : 'bg-slate-800 text-slate-300 border border-slate-700'}`}>
            {peer.status === 'requested' ? 'Requested' : peer.status === 'connected' ? 'Connected' : (peer.isSuggested ? 'Suggested' : 'Available')}
          </span>
        </div>
      ...
      <div className="mt-5">
        {peer.status === 'available' && (
          <button
            ...
            className="w-full py-3 rounded-2xl bg-cyan-500 text-slate-950 text-xs font-bold hover:bg-cyan-400 transition-colors"
          >
            Send Request
          </button>
        )}
        {peer.status === 'requested' && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-555 text-center">Waiting for acceptance.</p>
          </div>
        )}
  ...
  return (
    <div className="space-y-6">
      {viewPeer && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/90 p-4 overflow-hidden">
      ...
      <div className="space-y-4">
        <p className="text-xs uppercase tracking-[0.3em] text-cyan-400 font-semibold">Suggested Study Partners (Based on Weak Areas)</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {suggestedPeersList.map((peer) => renderPeerCard(peer))}
          {suggestedPeersList.length === 0 && (
            <div className="col-span-full p-8 text-center rounded-3xl bg-slate-950/40 border border-slate-900 text-slate-400 text-sm">
              No recommendations based on weak areas yet. Complete quizzes to get recommended study matches!
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-900">
        <p className="text-xs uppercase tracking-[0.3em] text-cyan-400 font-semibold">Search Other Peers</p>
        <input
          type="text"
          placeholder="Search other peers by name, expertise, or field..."
          value={searchQuery}
        ...
        />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {searchedOtherPeers.map((peer) => renderPeerCard(peer))}
          {searchedOtherPeers.length === 0 && (
            <div className="col-span-full p-8 text-center rounded-3xl bg-slate-950/40 border border-slate-900 text-slate-400 text-sm">
              No other study partners found matching your search.
            </div>
          )}
        </div>
      </div>
      ...
      {showChatModal && activePeer && activePeer.status === 'connected' && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/90 p-4 overflow-hidden">
          <div className="w-full max-w-6xl h-[85vh] max-h-[750px] rounded-3xl bg-[#07101f] border border-slate-800 p-6 shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6 flex-1 min-h-0 overflow-y-auto lg:overflow-hidden">
              <div className="flex flex-col h-[550px] lg:h-full min-h-0 justify-between gap-4">
                <div className="flex-1 overflow-y-auto rounded-3xl bg-slate-900/90 border border-slate-800 p-5 text-sm text-slate-200 overscroll-contain">
                  {messages.length === 0 ? (
                ...
                <div className="rounded-3xl bg-slate-900/90 border border-slate-800 p-5 shrink-0">
                  <textarea
                    value={chatInput}
              ...
              <div className="space-y-4 lg:overflow-y-auto lg:h-full lg:pr-2 overscroll-contain">
