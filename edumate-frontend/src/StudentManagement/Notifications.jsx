...
export default function Notifications({ notifications, markRead, markAllRead, clearNotification, clearAll }) {
  return (
    <div className="p-6 rounded-3xl bg-slate-950/90 border border-slate-800 shadow-xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="space-x-2">
          <div className="space-x-2">
            <button onClick={markAllRead} className="px-3 py-2 rounded-2xl bg-cyan-500/20 text-cyan-200 text-xs">Mark All Read</button>
            ...
            <div key={n.id} className={`p-3 rounded-2xl border ${n.unread ? 'border-cyan-500/30 bg-slate-900/80' : 'border-slate-800 bg-slate-950/80'}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-slate-300 mt-1">{n.message || n.text}</p>
                  <p className="text-xs text-slate-500 mt-1">{n.created_at ? new Date(n.created_at).toLocaleString() : (n.time || '')}</p>
                </div>
