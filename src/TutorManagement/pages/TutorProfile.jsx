...
const TutorProfile = () => {
  ...
  const [profile, setProfile] = useState({
    name: "Tutor Name",
    title: "A/L ICT Tutor",
    email: "tutor@edumate.lk",
    phone: "071 234 5678",
    qualification: "BSc in Computer Science",
    teachingArea: "Programming, Database Systems, Networking",
    about:
      "Experienced ICT tutor supporting A/L students with programming, database systems, networking, and exam-focused learning activities.",
    avatarUrl: "",
  });
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.18),transparent_30%),linear-gradient(135deg,#03111f,#020617)] text-white px-6 py-7">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr] mt-6">
        <div className="rounded-3xl border border-cyan-300/50 bg-[#041225]/80 p-7 shadow-[0_0_25px_rgba(34,211,238,0.45)] text-center">
          <div
            ...
            style={{
              backgroundImage: draft.avatarUrl
                ? `url(${draft.avatarUrl})`
                : "none",
          ...
          <div className="mt-5">
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-full bg-cyan-500/10 px-4 py-2 text-cyan-300 ring-1 ring-cyan-400/30 transition hover:bg-cyan-500/20"
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {!editMode ? <p>{profile[field]}</p> : null}
                  </div>
                </div>
                {editMode && (
                  <input
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-white outline-none focus:border-cyan-400 sm:w-72"
                    value={value}
      ...
      <div className="mt-6 rounded-3xl border border-cyan-300/50 bg-[#041225]/80 p-7 shadow-[0_0_25px_rgba(34,211,238,0.45)]">
        ...
        {editMode ? (
          <textarea
            className="mt-4 w-full min-h-[150px] rounded-3xl border border-slate-700 bg-slate-950/80 p-4 text-white outline-none focus:border-cyan-400"
            value={draft.about}
            onChange={handleChange("about")}
          />
        ) : (
          <p className="mt-5 text-slate-300 leading-7">{profile.about}</p>
        )}
      </div>
      ...
      {editMode && (
        <div className="mt-6 flex flex-wrap gap-3">
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-full bg-cyan-500/10 px-5 py-3 text-cyan-300 ring-1 ring-cyan-400/30 transition hover:bg-cyan-500/20"
            onClick={handleSave}
          >
            <CheckCircle size={18} />
            Save Changes
          ...
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-full bg-slate-900/90 px-5 py-3 text-slate-200 ring-1 ring-slate-700 transition hover:bg-slate-800"
